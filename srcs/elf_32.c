...
static void fill_symdata(t_symbol_data *sym_data, t_elf_32 elf_32) {
    ...
    for (int i = 0; i < elf_32.symbols_offset; i++) {
        ...
        elf_32.name = &elf_32.strtab[elf_32.symtab[i].st_name];
        ...
        if (ft_strlen(elf_32.name) != STT_NOTYPE && type != STT_FILE) {
            ...
            sym_data[sym_size].name = ft_strdup(elf_32.name);
            sym_size++;
        }
    }
...
int handle_elf_32(Elf32_Ehdr *file_hdr, u_int8_t *file_data) {
    ...
    elf_32.e_shstrndx = file_hdr->e_shstrndx;
    if (elf_32.e_shstrndx == 0)
        return print_error(options.file_name, " no symbols", NULL, false);
    else {
        elf_32.sections_hdr = (Elf32_Shdr *) (file_data + file_hdr->e_shoff);
        elf_32.shstrtab = get_strtab(file_data, elf_32.sections_hdr[elf_32.e_shstrndx].sh_size, elf_32.sections_hdr[elf_32.e_shstrndx].sh_offset);

        // recupere les addresses des headers symtab et strtab
        for (int i = 0; i < file_hdr->e_shnum; i++) {
            elf_32.name = &elf_32.shstrtab[elf_32.sections_hdr[i].sh_name];
            if (!ft_strncmp(elf_32.name, ".symtab", ft_strlen(".symtab")))
                elf_32.symtab_hdr = &elf_32.sections_hdr[i];
            else if (!ft_strncmp(elf_32.name, ".strtab", ft_strlen(".strtab")))
